class MultiWOZRunner(BaseRunner):
    ...
    def step_fn(self, inputs, entities, span_labels, belief_labels, resp_labels):
        ...
        entity_attention_mask = torch.where(entities == self.reader.pad_token_id, 0, 1)
        ...
        retriever_top_k_entity_indexes = retriever_entity_scores.sort(-1, True)[1][:, :5].unsqueeze(2)
        ...
        top_k_entity_mask = torch.gather(entity_attention_mask.unsqueeze(0).repeat(bsz, 1, 1), 1,
                                         retriever_top_k_entity_indexes.long().repeat(1, 1, num_entities))
    ...
    def train(self):
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            # print(entity_iterator.shape)

            self.train_epoch(train_iterator, optimizer, scheduler, reporter, entity_tensor)
